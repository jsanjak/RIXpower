...
my.haplo.pen.lmm = function (init.fit, data = sys.parent(), haplofreq = haplofreq, hap.base = hap.base,
                            ...
                            ...)
{
  ...
  init.est = init.fit$coef[,1] #just the coefficients
  p = length(init.est)-1
  cov.est = as.matrix(vcov(init.fit))# vcov for regression coefficients
  #print(cov.est)
